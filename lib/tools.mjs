...
export default async ({ recommend } = { recommend: true }) => {
    const tools = [
        {
    ...
    if (wrongVersionTools.length) {
        ...
        if (recommend) return; // default to skip this, as it is too opinionated to force updates
    ...
    if (!recommend && !missingTools.length && !wrongVersionTools.length) {
        echo(chalk.green('All required tools are installed with the recommended version'));
    }
}
